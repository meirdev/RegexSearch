...
/*
std::vector<std::string> Ack::parseToLines(const std::string& _fileContent, const SearchResult& _results, std::set<size_t>& _matchs)
{
    std::vector<std::string> lines;
    ...
    while (true)
    {
        ...
        while (beginResult+resultIndex != endResult && (beginResult+resultIndex)->m_start >= startIndex && (beginResult+resultIndex)->m_start <= found)
        {
            ...
            lineText << _fileContent.substr(startIndex, startResultIndex-startIndex);

            lineText << "\x1b[30m\x1b[43m" << _fileContent.substr(startResultIndex, endResultIndex - startResultIndex) << "\x1b[0m";

            if (beginResult+resultIndex+1 != _results.end() && (beginResult+resultIndex+1)->m_start < found)
            {
                lineText << _fileContent.substr(endResultIndex, (beginResult+resultIndex+1)->m_start - endResultIndex);

                endResultIndex += (beginResult+resultIndex+1)->m_start - endResultIndex;
            }
            else
            {
                lineText << _fileContent.substr(endResultIndex, found - endResultIndex);
            }
